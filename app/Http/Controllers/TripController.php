...
class TripController extends Controller
{
    ...
    public function fetchDataForOutput($usedIndexes,$breweriesData,$distanceLeft,$startLong,
        ...
        foreach ($usedIndexes as $index){
            ...
            $beerCount=$beerCount+$beerFound->count();
            $results[$i]['beer']=$beerFound;
            $results[$i]['latitude']=$breweriesData[$index]['latitude'];
            $results[$i]['longitude']=$breweriesData[$index]['longitude'];
            $results[$i]['distance']=$breweriesData[$index]['distance'];
            $i++;
        }
        ...
        if(is_array($results) && $results != null) {
            ...
            $results[$i]['latitude'] = $startLat;
            $results[$i]['longitude'] = $startLong;
            $results[$i]['distance'] = $breweriesData[end($usedIndexes)]['distanceFromHome'];
