...
@section('content')
    <div class="container">
        <div class="col-lg-12">
            ...
            {{-- subtract 4 additional fields in results array, 1 holds return home--}}
            <h5>Found {{count($results)-5}} breweries</h5>

            <h5>Distance traveled: {{$results['maxTripDistance']-$results['distanceLeft']}} km</h5>
            <br>
            @php($i=1)
            @foreach($results as $result)
                @if(is_array($result) && array_key_exists('longitude',$result) && $result['longitude']!=null)
                    {{--for other data in result array--}}
                    <p>{{$i++}}. @if($i-1==1) [HOME] @endif -> [ {{$result['brewery']['id']}}
                        ] {{$result['brewery']['name']}}
                        (lat/long)
